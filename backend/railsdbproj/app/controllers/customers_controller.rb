class CustomersController < ApplicationController
  ...
  def new_customer
    @customer = Customer.new(name: params[:name], email: params[:email], phonenumber: params[:phonenumber], issues: [params[:issues]], issue_description: params[:issue_description],
                             assignedusersid: [current_user.id])
